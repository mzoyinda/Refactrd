...
export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { name, company, email, what_to_build, business_challenge, stage, budget, timeline } = body;

    // Debug: log exactly what arrived
    console.log("Fields received:", { name: !!name, company: !!company, email: !!email, what_to_build: !!what_to_build, business_challenge: !!business_challenge, stage: !!stage, budget: !!budget, timeline: !!timeline });

    const missing = [];
    if (!name) missing.push("name");
    if (!company) missing.push("company");
    if (!email) missing.push("email");
    if (!what_to_build) missing.push("what_to_build");
    if (!business_challenge) missing.push("business_challenge");
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; background: #f9fafb; border-radius: 12px;">
          ...
          <div style="background: white; padding: 24px; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 16px;">
            <h2 style="color: #1F2A44; font-size: 16px; margin: 0 0 16px;">Contact Details</h2>
            <table style="width: 100%; border-collapse: collapse;">
              <tr><td style="padding: 8px 0; color: #64748B; font-size: 14px; width: 40%;">Name</td><td style="padding: 8px 0; color: #1F2A44; font-size: 14px; font-weight: 600;">${name}</td></tr>
              <tr><td style="padding: 8px 0; color: #64748B; font-size: 14px;">Company</td><td style="padding: 8px 0; color: #1F2A44; font-size: 14px; font-weight: 600;">${company}</td></tr>
              <tr><td style="padding: 8px 0; color: #64748B; font-size: 14px;">Email</td><td style="padding: 8px 0; color: #1F2A44; font-size: 14px; font-weight: 600;">${email}</td></tr>
              <tr><td style="padding: 8px 0; color: #64748B; font-size: 14px;">Stage</td><td style="padding: 8px 0; color: #1F2A44; font-size: 14px; font-weight: 600;">${stage}</td></tr>
              <tr><td style="padding: 8px 0; color: #64748B; font-size: 14px;">Budget</td><td style="padding: 8px 0; color: #1F2A44; font-size: 14px; font-weight: 600;">${budget}</td></tr>
              <tr><td style="padding: 8px 0; color: #64748B; font-size: 14px;">Timeline</td><td style="padding: 8px 0; color: #1F2A44; font-size: 14px; font-weight: 600;">${timeline}</td></tr>
            </table>
          </div>
          <div style="background: white; padding: 24px; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 16px;">
            <h2 style="color: #1F2A44; font-size: 16px; margin: 0 0 12px;">What They Want to Build</h2>
            <p style="color: #334155; font-size: 15px; line-height: 1.7; margin: 0 0 16px; background: #f8fafc; padding: 16px; border-radius: 6px; border-left: 4px solid #A2D2FF;">${what_to_build}</p>
            <h2 style="color: #1F2A44; font-size: 16px; margin: 16px 0 12px;">Business Challenge</h2>
            <p style="color: #334155; font-size: 15px; line-height: 1.7; margin: 0; background: #f8fafc; padding: 16px; border-radius: 6px; border-left: 4px solid #CBD5E1;">${business_challenge}</p>
          </div>
          <div style="text-align: center; padding: 16px;">
            <p style="color: #94a3b8; font-size: 13px; margin: 0;">Submission ID: ${data.id} · ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p>
          </div>
        </div>
      `,
    });
          ...
          <body style="margin: 0; padding: 0; background: #F4F6F9;">
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
              <div style="background: #1F2A44; padding: 32px 36px; border-radius: 12px 12px 0 0;">
                <p style="color: #A2D2FF; font-size: 12px; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; margin: 0 0 8px;">Refactrd · AI Engineering Studio</p>
                <h1 style="color: white; margin: 0; font-size: 26px; line-height: 1.3;">We have received your brief.</h1>
              </div>
              <div style="background: white; padding: 36px; border: 1px solid #E2E8F0;">
                <p style="color: #1F2A44; font-size: 17px; font-weight: 600; margin: 0 0 20px;">Hi ${name},</p>
                <p style="color: #475569; font-size: 15px; line-height: 1.8; margin: 0 0 16px;">Someone from the Refactrd team will review your brief personally and respond within 24 hours.</p>
                <p style="color: #475569; font-size: 15px; line-height: 1.8; margin: 0 0 16px;">We will either reach out to schedule a scoping call or, if a consultation is a better fit, we will explain why and point you in the right direction.</p>
                <p style="color: #475569; font-size: 15px; line-height: 1.8; margin: 0;">No auto-generated proposals. No generic responses. A real reply from a real person.</p>
              </div>
              <div style="background: #F8FAFC; padding: 24px 36px; border: 1px solid #E2E8F0; border-top: none;">
                <p style="color: #94A3B8; font-size: 11px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; margin: 0 0 10px;">Your brief</p>
                <p style="color: #334155; font-size: 14px; line-height: 1.8; margin: 0; padding-left: 14px; border-left: 3px solid #A2D2FF;">${what_to_build}</p>
              </div>
              <div style="background: #F4F6F9; padding: 20px 36px; border-radius: 0 0 12px 12px; border: 1px solid #E2E8F0; border-top: none; text-align: center;">
