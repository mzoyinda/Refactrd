...
const MINI_STEPS = [
  ...
  },
  {
    key: "team_size",
    label: "How many people are on your team?",
    options: ["Just me", "2 to 10 people", "11 to 50 people", "51 to 200 people", "200+ people"],
  },
  {
    key: "how_heard",
    label: "How did you hear about Refactrd?",
    options: ["LinkedIn", "Instagram", "Google Search", "Word of mouth", "A referral", "Newsletter"],
  },
];
...
const ENTERPRISE_STEPS = [
  {
    key: "industry",
    label: "What industry is your company in?",
    options: ["Fintech / Financial Services", "E-commerce / Retail", "Healthcare", "Legal", "Manufacturing / FMCG", "Media / Marketing", "SaaS / Technology", "Other"],
  },
  {
    key: "company_size",
    label: "How large is your organization?",
    options: ["50 to 200 people", "200 to 500 people", "500 to 1,000 people", "1,000+ people"],
  },
  {
    key: "org_readiness",
    label: "Where is your organization right now?",
  ...
  },
  {
    key: "how_heard",
    label: "How did you hear about Refactrd?",
    options: ["LinkedIn", "Google Search", "Word of mouth", "A referral", "Industry event", "Newsletter"],
  },
];
...
function PaymentMethodCard({
  ...
  return (
    <button
      onClick={onSelect}
      className={`w-full text-left px-4 py-4 rounded-xl border-2 transition-all duration-150 flex items-center gap-4 ${
        selected
          ? "border-[#1F2A44] bg-[#1F2A44] text-white"
    ...
    >
      ...
      <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 font-clash font-bold text-xs ${
        selected ? "bg-white/20 text-white" : isPaystack ? "bg-[#00C3F7]/10 text-[#00C3F7]" : "bg-[#F5A623]/10 text-[#F5A623]"
      }`}>
        {isPaystack ? "PAY" : "FLW"}
      </div>
      <div className="flex-1">
        <p className={`font-clash font-bold text-sm ${selected ? "text-white" : "text-[#1F2A44]"}`}>
          {isPaystack ? "Paystack" : "Flutterwave"}
        </p>
        <p className={`font-jakarta text-xs mt-0.5 ${selected ? "text-white/70" : "text-[#94A3B8]"}`}>
...
export default function ConsultationModal({ type, onClose }: Props) {
  const isMini = type === "mini";
  const USD_PRICE = isMini ? 99 : 499;
  const steps = isMini ? MINI_STEPS : ENTERPRISE_STEPS;
  const totalSteps = steps.length + 2; // questions + details + payment
      ...
      >
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <span className="inline-block px-3 py-1 bg-[#E6EAF0] text-[#1F2A44] text-xs font-clash font-bold rounded-full uppercase tracking-wide mb-2">
                ...
                {" · "}
                {fetchingRate ? (
                  <span className="opacity-50">loading price...</span>
                ) : (
                  <span>${USD_PRICE} · {ngnAmount ? formatNgn(ngnAmount) : ""}</span>
                )}
              </span>
              <h2 className="font-clash font-bold text-[#1F2A44] text-xl leading-tight">
                {loading ? "Setting up payment..." :
                 isPaymentStep ? "Choose payment method" :
                 isDetailsStep ? "Almost there" :
                 steps[currentStep]?.label}
              </h2>
            </div>
            <button
              onClick={onClose}
              className="ml-4 flex-shrink-0 w-8 h-8 rounded-full bg-[#F4F6F9] flex items-center justify-center text-[#64748B] hover:bg-[#E2E8F0] transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-2">
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-12">
              <Loader2 className="w-10 h-10 text-[#1F2A44] animate-spin mb-4" />
              <p className="font-clash font-semibold text-[#1F2A44] mb-1">Setting up your payment</p>
              <p className="font-jakarta text-sm text-[#94A3B8] text-center">
                Redirecting to {selectedProvider === "paystack" ? "Paystack" : "Flutterwave"}...
              </p>
            </div>
          )}

          {/* Question step */}
          {!loading && !isDetailsStep && !isPaymentStep && currentQ && (
            <div className="space-y-2 py-2">
              {steps[currentStep].options.map((option) => {
                const selected = answers[currentQ.key] === option;
                return (
          ...
          {!loading && isDetailsStep && (
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                <label className="font-clash font-bold text-sm text-[#1F2A44]">Full name *</label>
                <input
                  type="text"
                  value={details.full_name}
                  onChange={(e) => setDetails((p) => ({ ...p, full_name: e.target.value }))}
              ...
              <div className="space-y-2">
                <label className="font-clash font-bold text-sm text-[#1F2A44]">Work email *</label>
                <input
                  type="email"
                  value={details.email}
                  onChange={(e) => setDetails((p) => ({ ...p, email: e.target.value }))}
              ...
              <div className="space-y-2">
                <label className="font-clash font-bold text-sm text-[#1F2A44]">Company name *</label>
                <input
                  type="text"
                  value={details.company_name}
                  onChange={(e) => setDetails((p) => ({ ...p, company_name: e.target.value }))}
          ...
          {/* Payment method step */}
          {!loading && isPaymentStep && (
            <div className="space-y-4 py-2">
              {/* Amount display */}
              <div className="bg-[#1F2A44] rounded-xl p-4 text-center">
                ...
                <p className="font-jakarta text-white/50 text-xs mt-1">
                  ${USD_PRICE} · Rate: ₦{Math.round(rate).toLocaleString()}/USD (live)
                </p>
              </div>

              <p className="font-clash font-bold text-sm text-[#1F2A44]">Select payment method</p>

              <div className="space-y-2.5">
                <PaymentMethodCard
                  provider="paystack"
                  selected={selectedProvider === "paystack"}
                  onSelect={() => setSelectedProvider("paystack")}
                ...
                <PaymentMethodCard
                  ...
                  onSelect={() => setSelectedProvider("flutterwave")}
                  ngnAmount={ngnAmount || 0}
                  usdPrice={USD_PRICE}
                  rate={rate}
                />
            ...
            <div className="flex gap-3">
              ...
              {!isDetailsStep && !isPaymentStep ? (
                <button
                  onClick={handleNext}
                  disabled={!currentAnswer}
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-[#1F2A44] text-white rounded-full font-clash font-bold text-sm hover:bg-[#0e5d7d] disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200 group"
                >
                ...
                <button
                  onClick={handleSubmit}
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-[#1F2A44] text-white rounded-full font-clash font-bold text-sm hover:bg-[#0e5d7d] transition-all duration-200 group"
                >
                  Proceed to Payment
                  <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform duration-200" />
                </button>
